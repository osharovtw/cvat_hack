(self["webpackChunkcvat_ui"] = self["webpackChunkcvat_ui"] || []).push([["plugin_0"],{

/***/ "./plugins/sam_plugin/src/ts/index.tsx":
/*!*********************************************!*\
  !*** ./plugins/sam_plugin/src/ts/index.tsx ***!
  \*********************************************/
...
/* harmony import */ var onnxruntime_web__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! onnxruntime-web */ "../node_modules/onnxruntime-web/dist/ort-web.min.js");
...
/* harmony import */ var lru_cache__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! lru-cache */ "./node_modules/lru-cache/dist/mjs/index.js");
...
function modelData({
  ...
  // Create the tensor
  const pointCoordsTensor = new onnxruntime_web__WEBPACK_IMPORTED_MODULE_0__.Tensor('float32', pointCoords, [1, n + 1, 2]);
  const pointLabelsTensor = new onnxruntime_web__WEBPACK_IMPORTED_MODULE_0__.Tensor('float32', pointLabels, [1, n + 1]);
  const imageSizeTensor = new onnxruntime_web__WEBPACK_IMPORTED_MODULE_0__.Tensor('float32', [modelScale.height, modelScale.width]);
  const prevMask = maskInput || new onnxruntime_web__WEBPACK_IMPORTED_MODULE_0__.Tensor('float32', new Float32Array(256 * 256), [1, 1, 256, 256]);
  const hasMaskInput = new onnxruntime_web__WEBPACK_IMPORTED_MODULE_0__.Tensor('float32', [maskInput ? 1 : 0]);
  return {
...
const samPlugin = {
  ...
  cvat: {
    lambda: {
      call: {
        async enter(plugin, taskID, model, {
          frame
        }) {
        ...
        async leave(plugin, result, taskID, model, {
          frame,
          pos_points,
          neg_points
        }) {
          if (!plugin.data.task || plugin.data.task.id !== taskID) {
            [plugin.data.task] = await plugin.data.core.tasks.get({
              id: taskID
            });
          }
          const {
            height: imHeight,
            width: imWidth
          } = await plugin.data.task.frames.get(frame);
          const key = `${taskID}_${frame}`;
          if (model.id !== plugin.data.modelID) {
            return result;
          }
          if (result) {
            ...
            plugin.data.embeddings.set(key, new onnxruntime_web__WEBPACK_IMPORTED_MODULE_0__.Tensor('float32', float32Arr, [1, 256, 64, 64]));
          }
          const modelScale = {
            width: imWidth,
            height: imHeight,
          ...
          const feeds = modelData({
            ...
            tensor: plugin.data.embeddings.get(key),
            modelScale,
            maskInput: plugin.data.lowResMasks.has(key) ? plugin.data.lowResMasks.get(key) : null
          });
          function toMatImage(input, width, height) {
          ...
          function onnxToImage(input, width, height) {
            return toMatImage(input, width, height);
          }
          const data = await plugin.data.session.run(feeds);
          const {
            masks,
            low_res_masks: lowResMasks
          } = data;
  ...
  data: {
    ...
    embeddings: new lru_cache__WEBPACK_IMPORTED_MODULE_1__.LRUCache({
      // float32 tensor [256, 64, 64] is 4 MB, max 512 MB
      max: 128,
      updateAgeOnGet: true,
      updateAgeOnHas: true
    }),
    lowResMasks: new lru_cache__WEBPACK_IMPORTED_MODULE_1__.LRUCache({
...
const SAMModelPlugin = ({
  core
}) => {
  samPlugin.data.core = core;
  onnxruntime_web__WEBPACK_IMPORTED_MODULE_0__.InferenceSession.create(samPlugin.data.modelURL).then(session => {
...
function register() {
  if (Object.prototype.hasOwnProperty.call(window, 'cvatUI')) {
    window.cvatUI.registerComponent(SAMModelPlugin);
  }
}
window.addEventListener('plugins.ready', register, {
  once: true
});
...
/******/ var __webpack_exports__ = (__webpack_exec__("./plugins/sam_plugin/src/ts/index.tsx"));
/******/ }
]);
//# sourceMappingURL=../plugin_0.1c1ead59.map